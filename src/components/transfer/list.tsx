...
const List = ({ options, title, onChange, value }: TransferListProps) => {
  ...
  const styled = {
    ...
    header: stylex.props(styles.header({ outlineColor: theme.colors[ColorToken.Outline] }), typography.body.medium),
    title: stylex.props(styles.title),
  };

  return (
    <div className={clsx(classNames[TransferClassToken.List], styled.list.className)} style={styled.list.style}>
      <div className={clsx(classNames[TransferClassToken.Header], styled.header.className)} style={styled.header.style}>
        <span>{options.length}项</span>
        <span className={clsx(classNames[TransferClassToken.Title], styled.title.className)} style={styled.title.style}>
          {title}
        </span>
      </div>
